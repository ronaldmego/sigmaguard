  ...
  return (
    <div className="bg-white border border-gray-200 rounded-xl p-5">
      ...
      <div className="flex items-center justify-between mb-4">
        <h2 className="text-sm font-medium text-gray-500">Autonomous Agent</h2>
        <div className="flex items-center gap-2">
          <span
            className={`w-2 h-2 rounded-full ${
              isOperating
                ? "bg-brand-500 animate-live-pulse"
                : data?.status === "error"
                  ? "bg-red-500"
                  : "bg-gray-300"
            }`}
      ...
      {isRunning && countdown && (
        <div className="mb-4 px-3 py-2 bg-brand-50 border border-brand-200 rounded-lg">
          ...
          <p className="text-sm font-mono text-brand-700">{countdown}</p>
        </div>
      )}
      ...
      {isSimulationActive && (
        <div className="mb-4 px-3 py-2 bg-brand-50 border border-brand-200 rounded-lg flex items-center gap-2">
          <span className="w-2 h-2 rounded-full bg-brand-500 animate-live-pulse" />
          <p className="text-xs text-brand-700">Agent operating via simulation</p>
        </div>
      )}
